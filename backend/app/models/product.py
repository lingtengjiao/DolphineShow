...
class Product(Base):
    ...
    sample_price: Mapped[Decimal | None] = mapped_column(Numeric(10, 2))
    price_tiers: Mapped[list | None] = mapped_column(JSONB, default=list)
    min_order_qty: Mapped[int] = mapped_column(Integer, default=1)
    material: Mapped[str | None] = mapped_column(String(255))
    filling: Mapped[str | None] = mapped_column(String(255))
    size: Mapped[str | None] = mapped_column(String(255))
    weight: Mapped[str | None] = mapped_column(String(100))
    age_range: Mapped[str | None] = mapped_column(String(100))
    certifications: Mapped[list | None] = mapped_column(JSONB, default=list)
    support_customization: Mapped[bool] = mapped_column(Boolean, default=True)
    support_logo: Mapped[bool] = mapped_column(Boolean, default=True)
